...
function detectFlatBase(rows: OhlcEntry[], windowSize = 25): PatternResult {
  if (rows.length < windowSize) return { patternType: 'none', depthPct: null, lengthDays: null }
  const recent = rows.slice(-windowSize)
  const high = Math.max(...recent.map((r) => r.high))
  const low = Math.min(...recent.map((r) => r.low))
  const mid = (high + low) / 2
  if (mid <= 0) return { patternType: 'none', depthPct: null, lengthDays: null }
  const rangePct = ((high - low) / mid) * 100
  if (rangePct > 15) return { patternType: 'none', depthPct: null, lengthDays: null }
  ...
  if (firstHalfVol > 0 && secondHalfVol > firstHalfVol * 0.8) {
    ...
    if (rangePct > 8) return { patternType: 'none', depthPct: null, lengthDays: null }
  }
...
function detectCupHandle(rows: OhlcEntry[]): PatternResult {
  ...
  if (rows.length < 45) return { patternType: 'none', depthPct: null, lengthDays: null }
  for (let cupLen = 45; cupLen <= Math.min(90, rows.length - 5); cupLen++) {
    ...
    if (recoveryRatio < 0.92) continue  // must recover to within 8% of left high
...
function countBases(rows: OhlcEntry[]): number {
  ...
  while (i < rows.length - 10) {
    ...
    for (let winLen = 15; winLen <= 40 && i + winLen < rows.length; winLen++) {
      ...
      if (rangePct > 15) break  // not tight, no base
...
export async function GET(ctx: Context) {
  ...
  for (const [code, rows] of ohlcByCode.entries()) {
    ...
    if (pattern.patternType === 'none') pattern = detectCupHandle(rows)
    if (pattern.patternType === 'none') pattern = detectFlatBase(rows)

    if (patternFilter != null && patternFilter !== '' && patternFilter !== 'all') {
      if (pattern.patternType !== patternFilter) continue
    } else {
      if (pattern.patternType === 'none') continue
    }
  ...
  const patternOrder: Record<Types.BasePatternType, number> = { htf: 0, 'cup-handle': 1, flat: 2, none: 3 }
  results.sort((a, b) => {
