...
export default function StageAnalysisView({ onRowClick }: { onRowClick: (code: string) => void }) {
  ...
  const { data, loading, error, refetch } = Hooks.useStageAnalysis(stageFilter === 0 ? undefined : stageFilter)

  const rows = useMemo(() => data?.data ?? [], [data])
  const stageCounts = useMemo(() => {
    const counts: Record<number, number> = { 1: 0, 2: 0, 3: 0, 4: 0 }
    for (const r of data?.data ?? []) counts[r.stage] = (counts[r.stage] ?? 0) + 1
    return counts
  }, [data])
  ...
  return (
    <div className='idx-card'>
      ...
      <div className='idx-table-wrap'>
        <table className='idx-table'>
          ...
          <tbody>
            ...
            {!loading && rows.length === 0 && (
              <tr><td colSpan={10} className='idx-text-center idx-text-muted' style={{ padding: 24 }}>Tidak ada data</td></tr>
            )}
            {rows.map((r) => (
              <tr key={r.code} className='idx-row-clickable' onClick={() => onRowClick(r.code)}>
                ...
                <td className='idx-text-center'><StageBadge stage={r.stage} /></td>
                <td className='idx-text-right'>{Utils.Format.formatNum(r.price, 0)}</td>
                <td className='idx-text-right'>{r.ma50 != null ? Utils.Format.formatNum(r.ma50, 0) : '—'}</td>
                <td className='idx-text-right'>{r.ma150 != null ? Utils.Format.formatNum(r.ma150, 0) : '—'}</td>
                <td className='idx-text-right'>{r.ma200 != null ? Utils.Format.formatNum(r.ma200, 0) : '—'}</td>
                <td className='idx-text-right' style={{ color: (r.ma200SlopePct ?? 0) >= 0 ? 'var(--idx-up)' : 'var(--idx-down)' }}>
                  {r.ma200SlopePct != null ? `${r.ma200SlopePct > 0 ? '+' : ''}${Utils.Format.formatNum(r.ma200SlopePct, 2)}%` : '—'}
                </td>
      ...
      <div style={{ marginTop: 12, padding: '8px 0', borderTop: '1px solid var(--idx-border)', fontSize: 11, color: 'var(--idx-text-muted)' }}>
        Stage 1: Basing (MA200 flat) · Stage 2: Advancing (beli) · Stage 3: Topping (hati-hati) · Stage 4: Declining (hindari)
      </div>
    </div>
  )
}
