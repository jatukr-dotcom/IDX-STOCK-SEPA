...
export default function RsLineView({ onRowClick }: { onRowClick: (code: string) => void }) {
  ...
  const newHighCount = useMemo(() => (data?.data ?? []).filter((r) => r.rsLineNewHigh).length, [data])
  ...
  return (
    <div className='idx-card'>
      <div className='idx-card-header'>
        <div>
          ...
          <div className='idx-card-subtitle' style={{ fontSize: 12, color: 'var(--idx-text-muted)', marginTop: 2 }}>
            RS Line = Harga / IHSG · New High 52w: {newHighCount} saham · Total: {data?.totalCount ?? 0}
          </div>
        </div>
        ...
        <table className='idx-table'>
          ...
          <tbody>
            {loading && rows.length === 0 && (
              <tr><td colSpan={8} className='idx-text-center idx-text-muted' style={{ padding: 24 }}>Memuat...</td></tr>
            )}
            {!loading && rows.length === 0 && (
              <tr><td colSpan={8} className='idx-text-center idx-text-muted' style={{ padding: 24 }}>Tidak ada data</td></tr>
            )}
            {rows.map((r) => (
              <tr key={r.code} className='idx-row-clickable' onClick={() => onRowClick(r.code)}>
                ...
                <td className='idx-text-center'>
                  {r.rsLineNewHigh
                    ? <span style={{ color: 'var(--idx-up)', fontWeight: 700, fontSize: 13 }}>★ NEW HIGH</span>
                    : <span className='idx-text-muted' style={{ fontSize: 12 }}>—</span>}
                </td>
                <td className='idx-text-right'>{r.rsLineValue != null ? Utils.Format.formatNum(r.rsLineValue, 1) : '—'}</td>
                <td className='idx-text-right' style={{ color: (r.rsLinePctFrom52wHigh ?? -99) >= -2 ? 'var(--idx-up)' : 'var(--idx-text-muted)' }}>
                  {r.rsLinePctFrom52wHigh != null ? `${r.rsLinePctFrom52wHigh > 0 ? '+' : ''}${Utils.Format.formatNum(r.rsLinePctFrom52wHigh, 2)}%` : '—'}
                </td>
                <td className='idx-text-right'>{Utils.Format.formatNum(r.price, 0)}</td>
                <td className='idx-text-right' style={{ color: (r.pctFrom52wHigh ?? -99) >= -5 ? 'var(--idx-up)' : 'var(--idx-text-muted)' }}>
      ...
      <div style={{ marginTop: 12, padding: '8px 0', borderTop: '1px solid var(--idx-border)', fontSize: 11, color: 'var(--idx-text-muted)' }}>
        RS Line New High = RS Line saat ini ≥ RS Line 52w high · Leading indicator terkuat menurut Minervini, Ryan, Zanger, Ritchie
      </div>
    </div>
  )
}
