...
export default function BasePatternsView({ onRowClick }: { onRowClick: (code: string) => void }) {
  ...
  const counts = useMemo(() => {
    ...
    for (const r of data?.data ?? []) c[r.patternType] = (c[r.patternType] ?? 0) + 1
    return c
  }, [data])
  ...
  return (
    <div className='idx-card'>
          ...
          {(['all', 'htf', 'cup-handle', 'flat'] as FilterType[]).map((f) => (
            ...
            >
              {f === 'all' ? `Semua (${data?.totalCount ?? 0})` :
               f === 'htf' ? `HTF (${counts.htf ?? 0})` :
               f === 'cup-handle' ? `Cup (${counts['cup-handle'] ?? 0})` :
               `Flat (${counts.flat ?? 0})`}
            </button>
          ))}
          <button
        ...
        <table className='idx-table'>
          ...
          <tbody>
            {loading && rows.length === 0 && (
              <tr><td colSpan={9} className='idx-text-center idx-text-muted' style={{ padding: 24 }}>Memuat...</td></tr>
            )}
            {!loading && rows.length === 0 && (
              <tr><td colSpan={9} className='idx-text-center idx-text-muted' style={{ padding: 24 }}>Tidak ada pola terdeteksi</td></tr>
            )}
            {rows.map((r) => (
              <tr key={r.code} className='idx-row-clickable' onClick={() => onRowClick(r.code)}>
                ...
                <td><PatternBadge type={r.patternType} /></td>
                <td className='idx-text-center'><BaseCountBadge count={r.baseCount} /></td>
      ...
      <div style={{ marginTop: 12, padding: '8px 0', borderTop: '1px solid var(--idx-border)', fontSize: 11, color: 'var(--idx-text-muted)' }}>
        Base Count: makin kecil makin baik · 1st base probabilitas tertinggi · HTF = High Tight Flag (100%+ dalam 4-8 minggu lalu konsolidasi)
      </div>
    </div>
  )
}
